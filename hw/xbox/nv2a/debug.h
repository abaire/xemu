...
#define NV2A_PROF_ACCUMULATORS_XMAC \
    ...
    _X(NV2A_PROF_GL_DRAW_BEGIN__SURFACE_UPDATE) \
...
{
    int current_events = g_nv2a_stats.frame_working.accumulators[acc].events;
    g_nv2a_stats.frame_working.accumulators[acc].events += 1;
    g_nv2a_stats.frame_working.accumulators[acc].total += val;
    if (current_events == 0) {
        g_nv2a_stats.frame_working.accumulators[acc].min = val;
        g_nv2a_stats.frame_working.accumulators[acc].max = val;
    } else {
        if (val < g_nv2a_stats.frame_working.accumulators[acc].min) {
            g_nv2a_stats.frame_working.accumulators[acc].min = val;
        }
        if (val > g_nv2a_stats.frame_working.accumulators[acc].max) {
            g_nv2a_stats.frame_working.accumulators[acc].max = val;
        }
    }
}

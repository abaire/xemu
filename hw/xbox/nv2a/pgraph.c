...
static void pgraph_bind_shaders(PGRAPHState *pg)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (!f.linear && !cubemap && border_source != NV_PGRAPH_TEXFMT0_BORDER_SOURCE_COLOR) {
            // The actual texture will be (at least) double the reported
            // size and shifted by a 4 texel border but texture coordinates will
            // still be relative to the reported size.

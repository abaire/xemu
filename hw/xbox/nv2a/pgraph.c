...
static void pgraph_render_display(NV2AState *d, SurfaceBinding *surface)
{
    ...
    if (surface
        ...
        && surface->pitch == (width * framebuffer_bytes_per_pixel)) {
        ...
        unsigned int vga_pitch = vga_width * format->bytes_per_pixel;
        hwaddr framebuffer = d->pcrtc.start;
        size_t length = vga_pitch * vga_height;
    ...
    glUniform1i(pg->disp_rndr.tex_loc, 0);

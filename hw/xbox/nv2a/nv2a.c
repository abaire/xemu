...
{
    enum NV2A_DBG_STATE run_state =
        qatomic_read(&g_debugger_state.debugger_state);
    if (run_state != NV2A_DBG_RUNNING) {
        qatomic_set(&g_debugger_state.debugger_state, NV2A_DBG_RUNNING);
        qatomic_set(&g_debugger_state.device->pgraph.waiting_for_nv2a_debugger,
                    false);
        vm_start();
    }
...
void nv2a_dbg_handle_frame_swap(void)
{
    ...
    g_nv2a_info.draw_info.draw_ends_since_last_frame =
...
void nv2a_dbg_handle_begin_end(NV2ADbgDrawInfo *info)
{
    ...
    info->fixed_function =
        GET_MASK(pg->regs[NV_PGRAPH_CSV0_D], NV_PGRAPH_CSV0_D_MODE) == 0;
    info->lighting_enabled =
        GET_MASK(pg->regs[NV_PGRAPH_CSV0_C], NV_PGRAPH_CSV0_C_LIGHTING);
    ...
    qatomic_set(&g_debugger_state.draw_end_break_requested,
                draw_end_break_count);
    if (draw_end_break_count) {
        return;
    }
...
void nv2a_dbg_handle_generate_texture(GLuint texture, GLint internal_format,
...
void nv2a_dbg_handle_delete_texture(GLuint texture)
{
    ...
    //    printf("nv2a_dbg_handle_delete_texture: failed to delete texture
    //    info.\n");
}
...
NV2ADbgState *nv2a_dbg_fetch_state(void)
{
    ...
    for (int i = 0; i < NV2A_MAX_TEXTURES; ++i) {
        ...
        NV2ADebuggerTextureState *state =
            find_texture_info(binding->gl_texture);
        if (!tex_info) {

                           ...
                           const char *write_mask, bool is_alpha)
{
    ...
    if (mstring_get_length(sum_dest)) {
        mstring_append_fmt(ps->code, "mux_sum.%s = clamp(%s(%s), -1.0, 1.0);\n",
                           write_mask, caster, mstring_get_str(sum_mapping));
        assign_muxsum = true;
    }
    ...
    if (assign_cd) {
        ...
        if (!is_alpha && assign_cd && output.flags & PS_COMBINEROUTPUT_CD_BLUE_TO_ALPHA) {
    ...
    if (assign_muxsum) {
        mstring_append_fmt(ps->code, "%s.%s = mux_sum.%s;\n",
                           mstring_get_str(sum_dest), write_mask, write_mask);
    }
        ...
        case PS_TEXTUREMODES_PROJECT3D:
            if (ps->state.depth_tex_max[i] != 0.0f) {
                mstring_append_fmt(vars, "vec3 pT%dShadowCoord = pT%d.xyz / pT%d.w;\n",
                                   i, i, i, i);
                // OpenGL normalizes the depth texture to [0,1]; nv2a does not,
                //  so the z component must be fixed up.
